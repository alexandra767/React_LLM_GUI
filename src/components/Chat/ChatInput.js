...
import nativeSpeechService from '../../services/NativeSpeechService';
...
const ChatInput = React.forwardRef(({ onSendMessage, disabled }, ref) => {
  ...
  // Debug global paste events
  useEffect(() => {
    const debugPaste = (e) => {
      console.log('[ChatInput] Global paste event detected:', {
        target: e.target,
        targetTagName: e.target?.tagName,
        targetId: e.target?.id,
        isTextarea: e.target === inputRef.current,
    ...
    const debugKeydown = (e) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'v') {
        console.log('[ChatInput] Paste keyboard shortcut detected:', {
          isListening,
          target: e.target,
          prevented: false
        });
        
        // If we're listening, prevent the paste
        if (isListening) {
          e.preventDefault();
          e.stopPropagation();
          console.log('[ChatInput] Prevented paste keyboard shortcut during listening');
        }
      }
    };
    
    document.addEventListener('paste', debugPaste, true);
    document.addEventListener('keydown', debugKeydown, true);
    
    return () => {
      document.removeEventListener('paste', debugPaste, true);
      document.removeEventListener('keydown', debugKeydown, true);
    };
  }, [isListening]);
  
  // Debug voice service initialization
  React.useEffect(() => {
    console.log('[ChatInput] Voice service debug info:', {
      voiceService: !!voiceService,
      isSupported: voiceService?.isSupported(),
      recognition: voiceService?.recognition,
      synthesis: voiceService?.synthesis,
  ...
  React.useEffect(() => {
    console.log('[ChatInput] Message state changed to:', message);
    console.log('[ChatInput] Current textarea value:', inputRef.current?.value);
  }, [message]);
  ...
  const handleVoiceInput = async (e) => {
    // Prevent any default behavior that might trigger paste
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
    ...
    if (isListening) {
      ...
      console.log('[ChatInput] Stopped listening manually');
    } else {
      // Start listening
      try {
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
          try {
            ...
            if (navigator.permissions) {
              ...
              console.log('[ChatInput] Current microphone permission state:', micPermission.state);
            }
            ...
            console.log('[ChatInput] Microphone permission granted and stream obtained');
            console.log('[ChatInput] Stream tracks:', stream.getTracks().map(track => ({
              kind: track.kind,
              label: track.label,
            ...
            console.log('[ChatInput] Test stream stopped');
            ...
            const testStream = await navigator.mediaDevices.getUserMedia(audioConstraints);
        ...
        // Also clear the textarea directly to prevent any paste issues
        if (inputRef.current) {
          ...
          // Clear any selection that might trigger paste
          inputRef.current.setSelectionRange(0, 0);
          
          // Temporarily remove focus to clear any pending operations
          inputRef.current.blur();
          
          // Use a longer delay and ensure no paste can happen
          setTimeout(() => {
        ...
        if (window.electron && speechProvider === 'browser') {
          ...
          switch (speechProvider) {
            case 'azure':
              selectedService = azureSpeechService;
              console.log('[ChatInput] Using Azure Speech Service');
              break;
            case 'openai':
              selectedService = openAISpeechService;
              console.log('[ChatInput] Using OpenAI Speech Service');
              break;
            case 'vosk':
              selectedService = voskSpeechService;
              console.log('[ChatInput] Using Vosk Speech Service');
              break;
            case 'offline':
              selectedService = offlineVoiceService;
  ...
  const handleTextareaChange = (e) => {
    console.log('[ChatInput] Textarea change event triggered. Value:', e.target.value);
    console.log('[ChatInput] Message state before update:', message);
    setMessage(e.target.value);
    console.log('[ChatInput] Message state after update should be:', e.target.value);
      ...
      <form onSubmit={handleSubmit}>
        <InputWrapper>
          ...
          <Textarea
            ...
            onPaste={(e) => {
              console.log('[ChatInput] Paste event detected:', {
                isListening,
                clipboardData: e.clipboardData?.getData('text')
              });
              // If we're listening, prevent paste
              if (isListening) {
                e.preventDefault();
          ...
          <MicButton
            ...
            onMouseDown={(e) => {
              ...
              e.preventDefault();
              e.stopPropagation();
              console.log('[ChatInput] Microphone button mouse down');
            }}
            onMouseUp={(e) => {
              // Prevent any default mouse behavior
              e.preventDefault();
